...
export const ParcelAccordion = ({
  orderId, orderDisplayId, createdAt, total, currency_code = 'eur', orders
}: {
  orderId: string
  ...
  return (
    <>
      <div className="grid w-full grid-cols-2 rounded-sm border bg-component-secondary px-4 py-6 text-secondary sm:grid-cols-5" data-testid={`order-${orderId}`}>
        <div className="flex flex-col justify-between sm:col-span-4 sm:pr-10 lg:flex-row lg:items-center lg:gap-4">
          <h2 className="heading-sm truncate" data-testid="order-display-id">
            {t('orderSummary')} {orderDisplayId}
          </h2>
          <h2 className="label-md" data-testid="order-date">
            {tCommon('orderDate')}:{' '}
            <span className="text-primary lg:block xl:inline-block">{format(createdAt || '', 'yyyy-MM-dd')}</span>
          </h2>
          <h2 className="label-md" data-testid="order-total">
            {tCommon('total')}:{' '}
            <span className="text-primary lg:block xl:inline-block" data-testid={`order-${orderId}-price`}>
        ...
        <div className="col-span-1 flex items-center justify-end gap-4">
          <LocalizedClientLink href={`/user/orders/${orderId}`}>
            <Button variant="tonal" data-testid="order-view-button">
              <span className="label-md text-primary">{tCommon('viewOrder')}</span>
            </Button>
          </LocalizedClientLink>
        </div>
      </div>
