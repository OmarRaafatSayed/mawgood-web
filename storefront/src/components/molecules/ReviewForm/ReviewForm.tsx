...
export const ReviewForm: React.FC<Props> = ({ ...props }) => {
  const methods = useForm<ReviewFormData>({
    ...
    defaultValues: { sellerId: '', rating: 0, opinion: '' }
  });
...
const Form: FC<Props> = ({ handleClose, seller }) => {
  ...
  const { watch, handleSubmit, register, setValue, formState: { errors } } = useFormContext();
  ...
  return (
    <form onSubmit={handleSubmit(submit)} data-testid="review-form">
      <div className="space-y-4 px-4">
        <div className="items-top mb-4 grid max-w-full grid-cols-1 gap-4">
          <div>
            <label className="label-sm mb-2 block" data-testid="review-form-rating-label">
              {t('rating')}
            </label>
            ...
            {errors.rating?.message && (
              <p className="label-sm mt-1 text-negative" data-testid="review-form-rating-error">
          ...
          <label className={cn('label-sm relative block')}>
            <p className={cn(error && 'text-negative')} data-testid="review-form-opinion-label">
              {t('reviewText')}
            </p>
            <textarea
              className={cn(
            ...
            />
            <div
              className={cn('label-medium absolute right-4 text-secondary', errors.opinion?.message ? 'bottom-8' : 'bottom-3')}
              data-testid="review-form-character-count"
            >
              {`${lettersCount} / 300`}
            </div>
            {errors.opinion?.message && (
              <p className="label-sm text-negative" data-testid="review-form-opinion-error">
        ...
        {error && (
          <p className="label-md text-negative" data-testid="review-form-error">{error}</p>
        )}
        <Button className="w-full" data-testid="review-form-submit-button">
          {t('submitReview')}
        </Button>
      </div>
    </form>
