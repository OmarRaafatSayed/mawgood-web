...
export const SellerInfo = ({ seller, header = false, showArrow = false, bottomBorder = false }: {
  seller: SellerProps
  header?: boolean
  showArrow?: boolean
  ...
  const rating = reviews && reviews.length > 0
    ? reviews.filter((rev) => rev !== null).reduce((sum, r) => sum + r?.rating || 0, 0) / reviewCount
    : 0
    ...
    <div className="flex flex-col w-full">
      {showArrow ? (
        <LocalizedClientLink href={`/sellers/${seller.handle}`} aria-label={`View ${name} seller`}>
          <SellerInfoHeader photo={photo} name={name} rating={rating} reviewCount={reviewCount} showArrow={showArrow} bottomBorder={bottomBorder} />
        </LocalizedClientLink>
      ) : (
        <SellerInfoHeader photo={photo} name={name} rating={rating} reviewCount={reviewCount} showArrow={showArrow} bottomBorder={bottomBorder} />
      )}
      {!header && (
        <div className="flex flex-col gap-5 p-4">
          <h3 className="heading-sm uppercase">{t('sellerReviews')}</h3>
