...
const CartShippingMethodsSection: FC<ShippingProps> = ({ cart, availableShippingMethods }) => {
  ...
  const handleSetShippingMethod = async (id: string | null) => {
    ...
    try {
      ...
      setError(error?.message?.replace('Error setting up the request: ', '') || tCommon('error'));
    } finally {
      setIsLoadingPrices(false);
      router.refresh();
    }
  ...
  const groupedBySellerId = _shippingMethods?.reduce((acc: any, method) => {
    ...
    const amount = Number(method.price_type === 'flat' ? method.amount : calculatedPricesMap[method.id]);
    if (!isNaN(amount)) acc[sellerId]?.push(method);
    return acc;
  }, {});
  ...
  const renderListbox = (methods: any[], currentValue: string | undefined) => (
    <Listbox value={currentValue} onChange={value => { handleSetShippingMethod(value); }}>
      <div className="relative">
        <Listbox.Button className={clsx('text-base-regular relative flex h-12 w-full cursor-default items-center justify-between rounded-lg border bg-component-secondary px-4 text-left focus:outline-none focus-visible:border-gray-300 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-gray-300')}>
          {({ open }) => (
            <>
              <span className="block truncate">{tCommon('shippingMethod')}</span>
              <ChevronUpDown className={clx('transition-rotate duration-200', { 'rotate-180 transform': open })} />
            </>
          )}
        </Listbox.Button>
        <Transition as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
          <Listbox.Options className="text-small-regular border-top-0 absolute z-20 max-h-60 w-full overflow-auto rounded-lg border bg-white focus:outline-none sm:text-sm" data-testid="shipping-address-options">
            {methods.map((option: any) => (
              <Listbox.Option className="relative cursor-pointer select-none border-b py-4 pl-6 pr-10 hover:bg-gray-50" value={option.id} key={option.id}>
                {option.name}{' - '}
  ...
  return (
    <div className="bg-ui-bg-interactive rounded-sm border p-4">
      <div className="mb-6 flex flex-row items-center justify-between">
        <Heading level="h2" className="text-3xl-regular flex flex-row items-baseline gap-x-2">
                ...
                {filteredGroupedBySellerId.length === 0 && !hasGlobalMethods
                  ...
                  : (
                    <>
                      {hasGlobalMethods && (
                        <div className="mb-4">
                          <Heading level="h3" className="mb-2">{t('shippingDetails')}</Heading>
                          {renderListbox(globalShippingMethods, cart.shipping_methods?.[0]?.id)}
                        </div>
                      )}
                      {filteredGroupedBySellerId.map(key => (
                        <div key={key} className="mb-4">
                          <Heading level="h3" className="mb-2">{groupedBySellerId[key][0].seller_name}</Heading>
                          {renderListbox(groupedBySellerId[key], cart.shipping_methods?.[0]?.id)}
                        </div>
                      ))}
                    </>
                  )
              ...
              <div className="flex flex-col">
                {cart.shipping_methods?.map(method => (
                  <div key={method.id} className="mb-4 rounded-md border p-4">
                    <Text className="txt-medium-plus text-ui-fg-base mb-1">{tCommon('shippingMethod')}</Text>
