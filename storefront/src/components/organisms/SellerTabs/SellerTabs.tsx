...
export const SellerTabs = async ({
  tab, seller_handle, seller_id, locale, currency_code,
}: {
  tab: string
  ...
  const tabsList = [
    { label: t('products'), link: `/sellers/${seller_handle}/` },
    { label: t('reviews'), link: `/sellers/${seller_handle}/reviews` },
  ]
      ...
      <TabsContent value="products" activeTab={tab}>
        <Suspense fallback={<div data-testid="seller-tabs-products-loading"><ProductListingSkeleton /></div>}>
          {!ALGOLIA_ID || !ALGOLIA_SEARCH_KEY ? (
            ...
            <AlgoliaProductsListing locale={locale} seller_handle={seller_handle} currency_code={currency_code} />
          )}
        </Suspense>
      </TabsContent>
      <TabsContent value="reviews" activeTab={tab}>
        <Suspense fallback={<div data-testid="seller-tabs-reviews-loading">{t('reviews')}</div>}>
